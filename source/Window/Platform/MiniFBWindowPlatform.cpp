...
void MiniFBWindowPlatform::HandleResize(int width, int height) {
    ...
    _eventQueue.push(WindowResizeEvent{
        .width = _width,
        .height = _height
    });
    if (resizeCallback) {
...
void MiniFBWindowPlatform::HandleMouseButton(int button, int mod, bool isPressed) {
    _eventQueue.push(MouseButtonEvent{
...
void MiniFBWindowPlatform::HandleMouseScroll(int mod, float deltaX, float deltaY) {
    _scrollX = deltaX;
    _scrollY = deltaY;
    _eventQueue.push(MouseScrollEvent{
        .dx = deltaX,
        .dy = deltaY
    });
}
...
ModifierFlags MiniFBWindowPlatform::ConvertMods(int mfbMods) {
    ...
    if (mfbMods & KB_MOD_SHIFT) result = result | ModifierFlags::Shift;
    if (mfbMods & KB_MOD_CONTROL) result = result | ModifierFlags::Ctrl;
    if (mfbMods & KB_MOD_ALT) result = result | ModifierFlags::Alt;
    if (mfbMods & KB_MOD_SUPER) result = result | ModifierFlags::Super;
    if (mfbMods & KB_MOD_CAPS_LOCK) result = result | ModifierFlags::CapsLock;
    if (mfbMods & KB_MOD_NUM_LOCK) result = result | ModifierFlags::NumLock;
    return result;
}
...
MouseButton MiniFBWindowPlatform::ConvertMouseButton(int mfbButton) {
    ...
    switch (mfbButton) {
        case MOUSE_LEFT:   return MouseButton::Left;
        case MOUSE_RIGHT:  return MouseButton::Right;
        case MOUSE_MIDDLE: return MouseButton::Middle;
        case MOUSE_BTN_4:  return MouseButton::Button4;
        case MOUSE_BTN_5:  return MouseButton::Button5;
        case MOUSE_BTN_6:  return MouseButton::Button6;
        case MOUSE_BTN_7:  return MouseButton::Button7;
...
MiniFBWindowPlatform::MiniFBWindowPlatform(const WindowConfig& config, std::queue<WindowEvent>& eventQueue)
    ...
    unsigned flags = 0;
    if (config.resizable) flags |= WF_RESIZABLE;
    if (!config.visible) flags |= WF_BORDERLESS; // Best approximation
...
void MiniFBWindowPlatform::Present(const uint32_t* pixels, uint32_t width, uint32_t height) {
    if (!_window || !_isOpen) return;
    
    mfb_update_state state = mfb_update_ex(_window, const_cast<void*>(static_cast<const void*>(pixels)), width, height);
