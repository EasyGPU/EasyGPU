...
void KernelProfiler::PrintInfo(const std::string &mode) const {
	...
	if (_records.empty()) {
		std::cout << Col(COLOR_YELLOW) << "[KernelProfiler] " << Col(COLOR_RESET) << "No kernel executions recorded.\n";
		return;
	}
	...
	std::cout << Col(COLOR_CYAN) << "╔══════════════════════════════════════════════════════════════════════════════╗\n"
			  << Col(COLOR_RESET);
	std::cout << Col(COLOR_CYAN) << "║" << Col(COLOR_BOLD)
			  << "                    🚀  Kernel Profiling Results                               " << Col(COLOR_RESET)
			  << Col(COLOR_CYAN) << "║\n"
			  << Col(COLOR_RESET);
	std::cout << Col(COLOR_CYAN) << "╠══════════════════════════════════════════════════════════════════════════════╣\n"
			  << Col(COLOR_RESET);
	...
	if (mode == "trace") {
		...
		std::cout << Col(COLOR_CYAN) << "║ " << Col(COLOR_RESET) << Col(COLOR_BOLD) << std::left << std::setw(28)
				  << "Kernel" << Col(COLOR_RESET) << " │ " << std::right << std::setw(10) << "Time(ms)"
				  << " │ " << std::setw(10) << "Groups"
				  << " │ " << std::setw(16) << "Timestamp" << Col(COLOR_CYAN) << "   ║\n"
				  << Col(COLOR_RESET);
		...
		for (size_t i = 0; i < _records.size(); ++i) {
			...
			std::cout << Col(COLOR_CYAN) << "║ " << Col(COLOR_RESET) << rowColor << std::left << std::setw(28)
					  << record.kernelName.substr(0, 27) << Col(COLOR_RESET) << " │ " << std::right << Col(COLOR_GREEN)
					  << std::fixed << std::setprecision(3) << std::setw(10) << record.elapsedTimeMs << Col(COLOR_RESET)
					  << " │ " << std::setw(10) << groups << " │ " << std::setw(16) << timeStr << Col(COLOR_CYAN)
					  << "   ║\n"
					  << Col(COLOR_RESET);
		}
	} else {
		// Default: count mode - print statistics
		std::cout << Col(COLOR_CYAN) << "║ " << Col(COLOR_RESET) << Col(COLOR_BOLD) << std::left << std::setw(24)
				  << "Kernel" << Col(COLOR_RESET) << " │ " << std::right << std::setw(6) << "Count"
				  << " │ " << std::setw(9) << "Min(ms)"
				  << " │ " << std::setw(9) << "Avg(ms)"
				  << " │ " << std::setw(9) << "Max(ms)"
				  << " │ " << std::setw(10) << "Total(ms)"
				  << " │ " << std::setw(6) << "%" << Col(COLOR_CYAN) << "  ║\n"
				  << Col(COLOR_RESET);
		...
		std::sort(stats.begin(), stats.end(),
				  [](const auto &a, const auto &b) { return a.totalTimeMs > b.totalTimeMs; });
		...
		for (size_t i = 0; i < stats.size(); ++i) {
			...
			if (percent > 50)
				percentColor = Col(COLOR_RED);
			else if (percent > 20)
				percentColor = Col(COLOR_YELLOW);
			else if (percent > 5)
				percentColor = Col(COLOR_GREEN);

			std::cout << Col(COLOR_CYAN) << "║ " << Col(COLOR_RESET) << rowColor << std::left << std::setw(24)
					  << stat.kernelName.substr(0, 23) << Col(COLOR_RESET) << " │ " << std::right << std::setw(6)
					  << stat.counter << " │ " << std::fixed << std::setprecision(3) << std::setw(9) << stat.minTimeMs
					  << " │ " << Col(COLOR_GREEN) << std::setw(9) << stat.avgTimeMs << Col(COLOR_RESET) << " │ "
					  << std::setw(9) << stat.maxTimeMs << " │ " << Col(COLOR_MAGENTA) << std::setw(10)
					  << stat.totalTimeMs << Col(COLOR_RESET) << " │ " << percentColor << std::setw(5)
					  << std::setprecision(1) << percent << "%" << Col(COLOR_RESET) << Col(COLOR_CYAN) << "  ║\n"
					  << Col(COLOR_RESET);
		}
		...
		std::cout << Col(COLOR_CYAN)
				  ...
				  << Col(COLOR_RESET);
		std::cout << Col(COLOR_CYAN) << "║ " << Col(COLOR_RESET) << Col(COLOR_BOLD) << std::left << std::setw(24)
				  << "TOTAL" << Col(COLOR_RESET) << " │ " << std::right << std::setw(6) << _records.size() << " │ "
				  << std::setw(9) << ""
				  << " │ " << std::setw(9) << ""
				  << " │ " << std::setw(9) << ""
				  << " │ " << Col(COLOR_MAGENTA) << Col(COLOR_BOLD) << std::setw(10) << std::fixed
				  << std::setprecision(3) << totalTime << Col(COLOR_RESET) << " │ " << std::setw(6) << "100%"
				  << Col(COLOR_CYAN) << "  ║\n"
				  << Col(COLOR_RESET);
	}
...
std::string KernelProfiler::GetFormattedOutput(const std::string &mode) const {
	...
	if (mode == "trace") {
		...
		for (const auto &record : _records) {
			...
			oss << "| " << std::left << std::setw(28) << record.kernelName.substr(0, 27) << " | " << std::right
				<< std::fixed << std::setprecision(3) << std::setw(10) << record.elapsedTimeMs << " | " << std::setw(10)
				<< groups << " | " << std::setw(16) << timeStr << "   |\n";
		}
	} else {
		...
		for (const auto &stat : stats) {
			...
			oss << "| " << std::left << std::setw(24) << stat.kernelName.substr(0, 23) << " | " << std::right
				<< std::setw(6) << stat.counter << " | " << std::fixed << std::setprecision(3) << std::setw(9)
				<< stat.minTimeMs << " | " << std::setw(9) << stat.avgTimeMs << " | " << std::setw(9) << stat.maxTimeMs
				<< " | " << std::setw(10) << stat.totalTimeMs << " | " << std::setw(5) << std::setprecision(1)
				<< percent << "%" << "  |\n";
		}

		oss << "+------------------------------------------------------------------------------+\n";
		oss << "| " << std::left << std::setw(24) << "TOTAL"
			<< " | " << std::right << std::setw(6) << _records.size() << " | " << std::setw(9) << ""
			<< " | " << std::setw(9) << ""
			<< " | " << std::setw(9) << ""
			<< " | " << std::setw(10) << std::fixed << std::setprecision(3) << totalTime << " | " << std::setw(6)
			<< "100%" << "  |\n";
	}
...
KernelProfileScope::KernelProfileScope(const std::string &kernelName, int groupX, int groupY, int groupZ)
	: _kernelName(kernelName), _groupX(groupX), _groupY(groupY), _groupZ(groupZ), _queryId(0) {
