...
FragmentKernel2D::FragmentKernel2D(
	...
	const std::function<void(IR::Value::Var<GPU::Math::Vec2> &fragCoord, IR::Value::Var<GPU::Math::Vec2> &resolution,
...
FragmentKernel2D::FragmentKernel2D(FragmentKernel2D &&other) noexcept
	: _name(std::move(other._name)), _context(std::move(other._context)),
	  _windowAttachment(std::move(other._windowAttachment)), _vao(other._vao), _shaderProgram(other._shaderProgram),
	  _resourcesInitialized(other._resourcesInitialized), _width(other._width), _height(other._height) {
...
bool FragmentKernel2D::Attach(HWND hwnd) {
	...
	auto resizeCallback = [this](uint32_t width, uint32_t height) { this->OnResize(width, height); };
