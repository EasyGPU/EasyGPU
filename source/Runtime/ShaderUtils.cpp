...
std::string ShaderCompileException::FormatBeautifulError() const {
	...
	oss << Bold << Red << Vertical << Reset << " " << Red << CrossX << " Error: " << Reset
		<< displayMessage.substr(0, msgDisplayLen);
...
std::string ShaderErrorFormatter::MakeBox(const std::string &title, const std::vector<std::string> &lines,
	...
	oss << titleColor << Bold << Vertical << Reset << std::string(titlePadding, ' ') << Bold << title << Reset
		<< std::string(maxWidth - 2 - titlePadding - title.length(), ' ') << titleColor << Bold << Vertical << Reset
		<< "\n";
