...
GLStateCache::GLStateCache()
	: _currentProgram(0), _activeTextureUnit(0), _currentVAO(0), _hasAnySSBOBindings(false),
...
void GLStateCache::BindSSBO(uint32_t binding, GLuint buffer) {
	...
	if (_ssboBindings[binding] != buffer) {
		...
		if (buffer != 0) {
			...
			_hasAnySSBOBindings =
				std::any_of(_ssboBindings.begin(), _ssboBindings.end(), [](GLuint b) { return b != 0; });
		}
	}
}
...
void GLStateCache::BindImageTextures(const std::vector<std::pair<uint32_t, GLuint>> &bindings, GLenum format,
									 GLenum access) {
...
void GLStateCache::BindTexture(uint32_t unit, GLenum target, GLuint texture) {
	...
	if (_textureBindings[unit] != texture) {
		...
		if (texture != 0) {
			...
			_hasAnyTextureBindings =
				std::any_of(_textureBindings.begin(), _textureBindings.end(), [](GLuint t) { return t != 0; });
		}
	}
}
...
StateCacheInvalidateGuard::StateCacheInvalidateGuard(bool restoreOnExit) : _restoreOnExit(restoreOnExit) {
