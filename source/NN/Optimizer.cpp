		...
		for (const auto &ps : params_) {
			if (!ps.buffer)
				throw std::runtime_error(
					"Adam::Step now requires GPU-backed parameters; use AddTensor or pass a Buffer");
...
void GPUAdam::Step(AD::ADKernel1D &kernel, bool sync) {
	...
	for (auto &ps : params_) {
		...
		for (size_t i = 0; i < ps.size; i++) {
			...
			if (gp.gradHandle != first.gradHandle || gp.gradStride != first.gradStride ||
				gp.sampleCount != first.sampleCount || gp.gradOffset != first.gradOffset + static_cast<int>(i)) {
...
std::vector<GPUAdam::CombinedSlot> GPUAdam::BuildCombinedSlots(
	const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, size_t &totalSize) const {
...
std::string GPUAdam::CombinedSignature(const std::vector<CombinedSlot> &slots, size_t totalSize) const {
	std::string sig = std::to_string(totalSize);
	for (const auto &s : slots) {
		sig += std::format("|{}:{}:{}:{}:{}", s.base, s.size, s.sampleCount, s.gradOffset, s.gradStride);
	}
	return sig;
}
...
void GPUAdam::StepCombined(const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, bool sync) {
	size_t totalSize = 0;
...
void GPUAdam::EnsureCombinedPipeline(const std::vector<CombinedSlot> &slots, size_t totalSize) {
	...
	if (_combinedPipeline != Backend::INVALID_PIPELINE_HANDLE && _combinedSignature == sig)
		return;
...
std::string GPUAdam::BuildAdamShader(size_t paramSize, size_t sampleCount, int gradOffset, int gradStride) {
...
void main() {{
    ...
    weight[i] -= h[0] * mHat / (sqrt(vHat) + h[3]);
					   ...
					   paramSize, sampleCount, gradStride, gradOffset);
}

void GPUAdam::EnsurePipeline(ParamSlotGPU &ps, const AD::ADKernel1D::GradientParamInfo &first) {
	if (ps.pipeline != Backend::INVALID_PIPELINE_HANDLE && ps.compiledSamples == first.sampleCount &&
		ps.compiledGradOffset == first.gradOffset && ps.compiledGradStride == first.gradStride) {
		return;
	}
	...
	Backend::ShaderDesc shaderDesc;
	shaderDesc.type		  = Backend::ShaderType::Compute;
	shaderDesc.sourceCode = BuildAdamShader(ps.size, first.sampleCount, first.gradOffset, first.gradStride);
...
void GPUSGD::ValidateGradientGroup(const char *name, const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams,
	...
	for (size_t i = 0; i < size; i++) {
		...
		if (gp.gradHandle != first.gradHandle || gp.gradStride != first.gradStride ||
			...
			throw std::runtime_error(std::string(name) +
									 " requires tensor parameters to map to one contiguous gradient group");
		}
	}
}

std::vector<GPUSGD::CombinedSlot> GPUSGD::BuildCombinedSlots(
	const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, size_t &totalSize) const {
	std::vector<CombinedSlot> slots;
	slots.reserve(params_.size());
	totalSize		 = 0;
...
std::string GPUSGD::CombinedSignature(const std::vector<CombinedSlot> &slots, size_t totalSize) const {
	std::string sig = std::to_string(totalSize);
	for (const auto &s : slots) {
		sig += std::format("|{}:{}:{}:{}:{}", s.base, s.size, s.sampleCount, s.gradOffset, s.gradStride);
	}
...
std::string GPUSGD::BuildCombinedShader(const std::vector<CombinedSlot> &slots, size_t totalSize) {
	const size_t n	 = slots.size();
	std::string	 src = "#version 430\nlayout(local_size_x = 256) in;\n\n";
	for (size_t i = 0; i < n; i++) {
		src += std::format("layout(std430, binding = {}) buffer WeightBuf{} {{ float weight{}[]; }};\n", i, i, i);
...
void GPUSGD::StepCombined(const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, bool sync) {
	size_t totalSize = 0;
	auto   slots	 = BuildCombinedSlots(gradParams, totalSize);
	if (totalSize == 0)
		return;
...
void GPUSGD::EnsureCombinedPipeline(const std::vector<CombinedSlot> &slots, size_t totalSize) {
	...
	auto *backend = Runtime::Context::GetBackend();
	if (!backend)
		throw std::runtime_error("GPUSGD backend not available");
...
void GPUSGD::UploadCombinedHyperParams(size_t sampleCount) {
	...
	std::vector<float> h = {lr_, momentum_, weightDecay_, gradClip_,
							sampleCount == 0 ? 0.0f : 1.0f / static_cast<float>(sampleCount)};
	_combinedHyper->Upload(h);
}
...
std::string GPUSGD::BuildShader(size_t paramSize, size_t sampleCount, int gradOffset, int gradStride) {
	return std::format(R"GLSL(#version 430
layout(local_size_x = 256) in;
...
std::vector<GPURMSprop::CombinedSlot> GPURMSprop::BuildCombinedSlots(
	const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, size_t &totalSize) const {
	std::vector<CombinedSlot> slots;
	slots.reserve(params_.size());
	totalSize		 = 0;
...
std::string GPURMSprop::CombinedSignature(const std::vector<CombinedSlot> &slots, size_t totalSize) const {
	std::string sig = std::to_string(totalSize);
	for (const auto &s : slots) {
		sig += std::format("|{}:{}:{}:{}:{}", s.base, s.size, s.sampleCount, s.gradOffset, s.gradStride);
	}
...
std::string GPURMSprop::BuildCombinedShader(const std::vector<CombinedSlot> &slots, size_t totalSize) {
	const size_t n	 = slots.size();
	std::string	 src = "#version 430\nlayout(local_size_x = 256) in;\n\n";
	for (size_t i = 0; i < n; i++) {
		src += std::format("layout(std430, binding = {}) buffer WeightBuf{} {{ float weight{}[]; }};\n", i, i, i);
...
void GPURMSprop::StepCombined(const std::vector<AD::ADKernel1D::GradientParamInfo> &gradParams, bool sync) {
	size_t totalSize = 0;
	auto   slots	 = BuildCombinedSlots(gradParams, totalSize);
	if (totalSize == 0)
		return;
...
void GPURMSprop::EnsureCombinedPipeline(const std::vector<CombinedSlot> &slots, size_t totalSize) {
	...
	auto *backend = Runtime::Context::GetBackend();
	if (!backend)
		throw std::runtime_error("GPURMSprop backend not available");
...
void GPURMSprop::UploadCombinedHyperParams(size_t sampleCount) {
	...
	std::vector<float> h = {lr_,		  beta_,	 eps_,
							weightDecay_, gradClip_, sampleCount == 0 ? 0.0f : 1.0f / static_cast<float>(sampleCount)};
	_combinedHyper->Upload(h);
}
...
std::string GPURMSprop::BuildShader(size_t paramSize, size_t sampleCount, int gradOffset, int gradStride) {
	return std::format(R"GLSL(#version 430
layout(local_size_x = 256) in;
...
void SGD::Step(AD::ADKernel1D &kernel) {
	if (!gpu_) {
		...
		for (const auto &ps : params_) {
			if (!ps.buffer)
				throw std::runtime_error(
					"SGD::Step now requires GPU-backed parameters; use AddTensor or pass a Buffer");
			gpu_->AddParameter(ps.size, ps.buffer->GetHandle());
		}
	}
	gpu_->SetLearningRate(lr_);
...
void RMSprop::Step(AD::ADKernel1D &kernel) {
	if (!gpu_) {
		...
		for (const auto &ps : params_) {
			if (!ps.buffer)
				throw std::runtime_error(
					"RMSprop::Step now requires GPU-backed parameters; use AddTensor or pass a Buffer");
			gpu_->AddParameter(ps.size, ps.buffer->GetHandle());
		}
	}
	gpu_->SetLearningRate(lr_);
