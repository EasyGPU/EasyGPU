...
IfChain::IfChain(std::unique_ptr<IR::Node::Node> condition, std::vector<std::string> ifCode,
				 IR::Builder::BuilderContext *originalContext)
	: _originalContext(originalContext), _emitted(false) {
...
IfChain::IfChain(IfChain &&other) noexcept
	: _conditions(std::move(other._conditions)), _codeBlocks(std::move(other._codeBlocks)),
	  _elseCode(std::move(other._elseCode)), _originalContext(other._originalContext), _emitted(other._emitted) {
