...
namespace GPU::Flow {
CodeCollectContext::CodeCollectContext() : _parentContext(nullptr) {
}
...
void CodeCollectContext::RegisterBuffer(uint32_t binding, const std::string &typeName, const std::string &bufferName,
										int mode) {
	if (_parentContext) {
...
void CodeCollectContext::RegisterTexture(uint32_t binding, Runtime::PixelFormat format, const std::string &textureName,
										 uint32_t width, uint32_t height, bool sampled) {
	if (_parentContext) {
		_parentContext->RegisterTexture(binding, format, textureName, width, height, sampled);
	}
}
...
void CodeCollectContext::PushCallableBody(const std::string &callableName,
	...
	if (_parentContext) {
		_parentContext->PushCallableBody(callableName, parameterNames);
	}
}
