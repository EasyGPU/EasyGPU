...
void VulkanBackend::CleanupVulkan() {
    if (_device) {
        ...
        for (auto& [handle, info] : _pipelines) {
            if (info.pipeline) vkDestroyPipeline(_device, info.pipeline, nullptr);
            if (info.layout) vkDestroyPipelineLayout(_device, info.layout, nullptr);
            if (info.descriptorSetLayout) vkDestroyDescriptorSetLayout(_device, info.descriptorSetLayout, nullptr);
        }
        _pipelines.clear();
        
        // Destroy shaders
        for (auto& [handle, info] : _shaders) {
            if (info.module) vkDestroyShaderModule(_device, info.module, nullptr);
        }
        _shaders.clear();
        
        // Destroy textures
        for (auto& [handle, info] : _textures) {
            if (info.view) vkDestroyImageView(_device, info.view, nullptr);
            if (info.image) {
                vkDestroyImage(_device, info.image, nullptr);
            }
            if (info.memory) vkFreeMemory(_device, info.memory, nullptr);
        }
        _textures.clear();
        ...
        for (auto& [handle, info] : _buffers) {
            ...
            if (info.buffer) vkDestroyBuffer(_device, info.buffer, nullptr);
            if (info.stagingBuffer) vkDestroyBuffer(_device, info.stagingBuffer, nullptr);
            if (info.memory) vkFreeMemory(_device, info.memory, nullptr);
            if (info.stagingMemory) vkFreeMemory(_device, info.stagingMemory, nullptr);
        }
        _buffers.clear();
        ...
        // Destroy query pool
        if (_queryPool) vkDestroyQueryPool(_device, _queryPool, nullptr);
        
        // Destroy command resources
        if (_commandFence) vkDestroyFence(_device, _commandFence, nullptr);
...
void VulkanBackend::InvalidateAllDescriptorCaches() {
    ...
    if (!setsToFree.empty()) {
        CheckVkResult(vkFreeDescriptorSets(_device, _descriptorPool,
...
void VulkanBackend::InvalidateDescriptorCachesForPipeline(PipelineHandle pipeline) {
    std::vector<VkDescriptorSet> setsToFree;
    auto eraseBegin = std::remove_if(_descriptorSets.begin(), _descriptorSets.end(),
                                     [&](const DescriptorSetCache& cache) {
...
void VulkanBackend::AllocateBufferMemory(VkBuffer buffer, VkDeviceMemory& memory, 
...
void VulkanBackend::AllocateImageMemory(VkImage image, VkDeviceMemory& memory, 
...
void VulkanBackend::DestroyTexture(TextureHandle texture) {
    ...
    if (it->second.view) vkDestroyImageView(_device, it->second.view, nullptr);
    if (it->second.image) vkDestroyImage(_device, it->second.image, nullptr);
    if (it->second.memory) vkFreeMemory(_device, it->second.memory, nullptr);
    
    _textures.erase(it);
}

void VulkanBackend::UploadTexture(TextureHandle texture, uint32_t x, uint32_t y,
...
void VulkanBackend::UploadTextureInternal(TextureInfo& info, uint32_t x, uint32_t y,
                                           uint32_t width, uint32_t height, const void* data) {
    ...
    switch (info.format) {
        case PixelFormat::R8: pixelSize = 1; break;
        case PixelFormat::RG8: pixelSize = 2; break;
        case PixelFormat::RGBA8: pixelSize = 4; break;
        case PixelFormat::R32F: pixelSize = 4; break;
        case PixelFormat::RG32F: pixelSize = 8; break;
        case PixelFormat::RGBA32F: pixelSize = 16; break;
        case PixelFormat::R16F: pixelSize = 2; break;
        case PixelFormat::RG16F: pixelSize = 4; break;
        case PixelFormat::RGBA16F: pixelSize = 8; break;
        case PixelFormat::R32I: pixelSize = 4; break;
        case PixelFormat::RG32I: pixelSize = 8; break;
        case PixelFormat::RGBA32I: pixelSize = 16; break;
        case PixelFormat::R32UI: pixelSize = 4; break;
        case PixelFormat::RG32UI: pixelSize = 8; break;
        case PixelFormat::RGBA32UI: pixelSize = 16; break;
    }
    ...
    TransitionTexture(info, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    vkCmdCopyBufferToImage(_commandBuffer, stagingBuffer, info.image,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

    TransitionTexture(info, VK_IMAGE_LAYOUT_GENERAL,
                      VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
...
void VulkanBackend::DownloadTexture(TextureHandle texture, uint32_t x, uint32_t y,
                                     uint32_t width, uint32_t height, void* outData) {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    TransitionTexture(it->second, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    vkCmdCopyImageToBuffer(_commandBuffer, it->second.image,
...
VkFormat VulkanBackend::GetVkFormat(PixelFormat format) {
    switch (format) {
        ...
        case PixelFormat::R32I: return VK_FORMAT_R32_SINT;
        case PixelFormat::RG32I: return VK_FORMAT_R32G32_SINT;
        case PixelFormat::RGBA32I: return VK_FORMAT_R32G32B32A32_SINT;
        case PixelFormat::R32UI: return VK_FORMAT_R32_UINT;
        case PixelFormat::RG32UI: return VK_FORMAT_R32G32_UINT;
        case PixelFormat::RGBA32UI: return VK_FORMAT_R32G32B32A32_UINT;
...
VkDescriptorType VulkanBackend::GetVkDescriptorType(BindingType type) {
    switch (type) {
        case BindingType::Buffer: return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        case BindingType::Texture: return VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
...
VkShaderStageFlags VulkanBackend::GetVkShaderStage(ShaderType type) {
    switch (type) {
        ...
        default: return VK_SHADER_STAGE_COMPUTE_BIT;
    }
}
...
PipelineHandle VulkanBackend::CreatePipeline(const PipelineDesc& desc) {
    ...
    result = vkCreateComputePipelines(_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline);
    if (result != VK_SUCCESS) {
...
void VulkanBackend::DestroyPipeline(PipelineHandle pipeline) {
    ...
    if (it->second.pipeline) vkDestroyPipeline(_device, it->second.pipeline, nullptr);
    if (it->second.layout) vkDestroyPipelineLayout(_device, it->second.layout, nullptr);
    if (it->second.descriptorSetLayout) vkDestroyDescriptorSetLayout(_device, it->second.descriptorSetLayout, nullptr);
...
void VulkanBackend::TransitionTexture(TextureInfo& info, VkImageLayout newLayout,
...
void VulkanBackend::SetUniform(PipelineHandle pipeline, const std::string& name,
                                const std::string& type, const void* data) {
    (void)pipeline;
    (void)name;
    (void)type;
    (void)data;
    throw std::runtime_error("Vulkan backend does not implement SetUniform yet; use buffers or add a real uniform path");
}
...
void VulkanBackend::SetUniformData(PipelineHandle pipeline, const void* data, size_t size) {
    ...
    EnsureCommandBuffer();
    vkCmdPushConstants(_commandBuffer, it->second.layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
...
VulkanBackend::DescriptorSetCache* VulkanBackend::FindOrCreateDescriptorSet(const ResourceBinding* bindings, uint32_t count) {
    ...
    for (auto& cache : _descriptorSets) {
        ...
        bool matches = true;
        for (uint32_t i = 0; i < MAX_BUFFER_BINDINGS; ++i) {
            if ((requested.bufferMask & (1ull << i)) != 0 &&
                cache.boundBuffers[i] != requested.boundBuffers[i]) {
                matches = false;
                break;
            }
        }
...
void VulkanBackend::BindResources(const ResourceBinding* bindings, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        auto layoutIt = std::find_if(pipelineInfo.resources.begin(), pipelineInfo.resources.end(),
        ...
        if (binding.type == BindingType::Buffer) {
            ...
            if (binding.type == BindingType::Sampler) {
                ...
                const VkAccessFlags shaderAccess = layoutIt->readOnly
                    ...
                    : (VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
                TransitionTexture(it->second, VK_IMAGE_LAYOUT_GENERAL,
                                  VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, shaderAccess);
            }
        }
    }
    ...
    vkCmdBindDescriptorSets(_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                            pipelineInfo.layout, 0, 1, &cache->set, 0, nullptr);
}
...
void VulkanBackend::MemoryBarrier(BarrierType barrierType) {
    ...
    if (memoryBarrier.srcAccessMask != 0 || memoryBarrier.dstAccessMask != 0) {
        vkCmdPipelineBarrier(_commandBuffer, srcStage, dstStage, 0,
                             1, &memoryBarrier,
                             0, nullptr,
                             0, nullptr);
    }
...
uint64_t VulkanBackend::EndQuery(uint32_t query) {
    ...
    VkResult result = vkGetQueryPoolResults(_device, _queryPool, queryIndex, 2,
    ...
    double elapsedNanoseconds = static_cast<double>(timestamps[1] - timestamps[0]) * static_cast<double>(_timestampPeriod);
