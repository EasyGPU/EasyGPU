...
void* OpenGLBackend::MapBuffer(BufferHandle buffer, bool read, bool write) {
    ...
    GLbitfield access = 0;
    if (read) access |= GL_MAP_READ_BIT;
    if (write) access |= GL_MAP_WRITE_BIT;
...
void OpenGLBackend::UploadTexture(TextureHandle texture, uint32_t x, uint32_t y,
...
void OpenGLBackend::DownloadTexture(TextureHandle texture, uint32_t x, uint32_t y,
...
void OpenGLBackend::SetUniform(PipelineHandle pipeline, const std::string& name,
...
void OpenGLBackend::CreateHiddenWindow() {
    _hwnd = CreateWindowExW(0, s_windowClassName, L"EasyGPU Context", WS_OVERLAPPEDWINDOW,
                            CW_USEDEFAULT, CW_USEDEFAULT, 1, 1, nullptr, nullptr, 
                            _hInstance, nullptr);
