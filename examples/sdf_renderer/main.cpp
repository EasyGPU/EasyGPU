...
Callable<Float(Float3, Float3)> intersect_light = [](Float3 pos, Float3 d) {
	...
	Float  result;
	If(valid, [&]() { result = Expr<float>(D); }).Else([&]() { result = INF; });
	Return(result);
};
...
Callable<Float3(Float3, Int &)> out_dir = [](Float3 n, Int &seed) {
	Float3 u;
	If(Abs(n.y()) < 1.0f - EPS, [&]() { u = Normalize(Cross(n, MakeFloat3(0.0f, 1.0f, 0.0f))); }).Else([&]() {
...
Callable<Int(Float)> FloatToInt = [](Float f) {
	Int result = ToInt(f);
	If(f < 0.0f, [&]() { result = result - MakeInt(1); });
	Return(result);
};
...
Callable<Float(Float)> make_nested = [](Float f) {
	...
	If(f < 0.0f, [&]() {
		...
		If(f_int % 2 == 0, [&]() { result = 1.0f - fract_val; }).Else([&]() { result = Expr<float>(fract_val); });
	}).Else([&]() { result = Expr<float>(f); });
...
// Find next hit
Callable<void(Float &, Float3 &, Float3 &, Float3, Float3)> next_hit = [](Float &closest, Float3 &normal, Float3 &c,
																		  Float3 pos, Float3 d) {
	closest				 = INF;
	...
	If(ray_march_dist < Min(DIST_LIMIT, closest), [&]() {
		...
		If(t == 0, [&]() {
		  ...
		  }).Else([&]() { c = base_color + MakeFloat3(pattern.z(), pattern.y(), pattern.x()) * 1.0f; });
	});
};
