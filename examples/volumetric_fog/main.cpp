...
Callable<Float(Float3)> ValueNoise = [](Float3 &p) {
	...
	For(0, 8, [&](Int &idx) {
		...
		Float  weight	 = (1.0f - Abs(f.x() - dx)) * (1.0f - Abs(f.y() - dy)) * (1.0f - Abs(f.z() - dz));
...
Callable<Float(Float3)> GetDensity = [](Float3 &pos) {
	...
	// Base shape falloff
	Float  baseShape;
	If(dist < 1.0f, [&]() { baseShape = Pow(1.0f - dist, 2.0f); }).Else([&]() { baseShape = 0.0f; });
...
Callable<Float3(Float3, Float3)> RenderVolume = [](Float3 &rayOrigin, Float3 &rayDir) {
	...
	If(discriminant < 0.0f, [&]() { Return(bgColor); });
	...
	For(0, MAX_STEPS, [&](Int &) {
		If(t > tFar || transmittance < 0.001f, [&]() { Break(); });
...
int main() {
	...
	Kernel::Kernel2D kernel([&](Int &px, Int &py) {
		...
		Float3 rayDir = Normalize(MakeFloat3(cameraForward) + MakeFloat3(cameraRight) * u + MakeFloat3(cameraUp) * v);
