...
static std::vector<int> tokenize(const std::string &name, const Vocabulary &vocab, size_t maxLen) {
...
static std::vector<float> cpuLinear(const std::vector<float> &x, const std::vector<float> &W, size_t outFeatures,
									size_t inFeatures) {
...
static int cpuAttention(const std::vector<float> &q, const std::vector<std::vector<float>> &keys,
						const std::vector<std::vector<float>> &values, size_t headDim) {
		...
		Tensor<float, VOCAB_SIZE, N_EMBD>					lmHeadTensor;
		{
			...
			for (size_t j = 0; j < V; j++)
				for (size_t i = 0; i < N_EMBD; i++) {
					...
					lmData[j * N_EMBD + i] = (static_cast<float>(s) / UINT32_MAX * 2.0f - 1.0f) * range;
				}
		...
		for (size_t step = 0; step < NUM_STEPS; step++) {
			...
			for (size_t b = 0; b < BATCH_SIZE; b++) {
				...
				for (size_t pos = 0; pos < static_cast<size_t>(SEQ); pos++) {
					long long src			 = static_cast<long long>(targetPos) - static_cast<long long>(BLOCK_SIZE) +
											   static_cast<long long>(pos);
					tokenData[b * SEQ + pos] = src < 0 ? static_cast<int>(vocab.bos) : toks[static_cast<size_t>(src)];
				}
			}
