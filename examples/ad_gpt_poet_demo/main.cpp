...
static char idxToChar(int idx) {
	if (idx < 0 || idx >= static_cast<int>(VOCAB_SIZE)) return '?';
	return VOCAB[idx];
}
...
static std::vector<int> encode(const std::string &text) {
	...
	for (char c : text) ids.push_back(charToIdx(c));
	return ids;
}
...
static std::vector<float> cpuSoftmax(const std::vector<float> &logits) {
	...
	for (size_t i = 0; i < logits.size(); i++) {
		...
		probs[i] = std::exp(z);
		if (!std::isfinite(probs[i]) || probs[i] < 0.0f) probs[i] = 0.0f;
		sum += probs[i];
	}
	...
	for (float &p : probs) p = static_cast<float>(static_cast<double>(p) / sum);
	return probs;
}
...
static uint32_t ngramKey4(int a, int b, int c, int d) {
	return static_cast<uint32_t>(((a * static_cast<int>(VOCAB_SIZE) + b)
		* static_cast<int>(VOCAB_SIZE) + c) * static_cast<int>(VOCAB_SIZE) + d);
}

static std::string tokenKey(const std::vector<int> &tokens, size_t start, size_t count) {
	std::string key(count, '\0');
	for (size_t i = 0; i < count; i++) key[i] = static_cast<char>(tokens[start + i]);
	return key;
}
...
int main() {
	try {
		...
		for (size_t i = 0; i + 1 < ids.size(); i++) bigramLog[ids[i]][ids[i + 1]] += 1.0f;
		for (auto &row : bigramLog) {
			float sum = 0.0f;
			for (float v : row) sum += v;
			for (float &v : row) v = std::log(v / sum);
		}
		std::vector<std::vector<float>> trigramLog(VOCAB_SIZE * VOCAB_SIZE,
												   std::vector<float>(VOCAB_SIZE, 0.25f));
		...
		for (auto &kv : fivegramLog) {
			...
			for (float &v : kv.second) v = std::log(v / sum);
		}
		...
		Tensor<float, VOCAB_SIZE, N_EMBD> lmHeadTensor;
		{
			...
			for (size_t j = 0; j < VOCAB_SIZE; j++) {
				for (size_t i = 0; i < N_EMBD; i++) {
					...
					lmData[j * N_EMBD + i] =
						(static_cast<float>(seed) / UINT32_MAX * 2.0f - 1.0f) * range;
				}
			}
		...
		ADKernel1D kernel([&](Var<int> &batchIdx) {
			...
			Flow::For(MakeInt(0), MakeInt(B), [&](Var<int> &pos) {
				...
				totalLoss = totalLoss + CrossEntropyLoss(data, static_cast<int>(VOCAB_SIZE),
														 targetId, posLogBase);
			});
		...
		std::printf("Params: %zu, Tape: %zu, Shader: %zu bytes\n\n",
					kernel.ParameterCount(), kernel.Tape().Size(), kernel.CombinedCode().size());
		...
		for (size_t step = 0; step < NUM_STEPS; step++) {
			...
			if (step == 0) {
				...
				for (const auto &g : allG) {
					double s = 0.0;
					for (float v : g) s += std::abs(static_cast<double>(v));
		...
		bool weightsFinite = tensorIsFinite(tokEmb.Weight().Data(), tokEmb.Weight().Size())
		...
		auto runContext = [&](const std::vector<int> &context) {
			...
			for (size_t pos = 0; pos < BLOCK_SIZE; pos++) {
				...
				for (float &v : hidden) v = std::max(v, 0.0f);
		...
		for (int i = 0; i < GENERATE_CHARS; i++) {
			...
			for (size_t j = 0; j < keep; j++) context[dstStart + j] = history[srcStart + j];
			...
			size_t fourRow = (static_cast<size_t>(prev3) * VOCAB_SIZE + static_cast<size_t>(prev2))
				* VOCAB_SIZE + static_cast<size_t>(prev);
			...
			for (size_t v = 0; v < VOCAB_SIZE; v++)
				logits[v] = GPT_WEIGHT * std::log(std::max(gptProbs[v], 1e-8f))
					+ BIGRAM_WEIGHT * bigramLog[prev][v]
					+ TRIGRAM_WEIGHT * trigramLog[triRow][v]
					+ FOURGRAM_WEIGHT * fourgramLog[fourRow][v];
			if (fiveIt != fivegramLog.end()) {
				for (size_t v = 0; v < VOCAB_SIZE; v++) logits[v] += FIVEGRAM_WEIGHT * fiveIt->second[v];
			}
			if (longIt != longContextLog.end()) {
				for (size_t v = 0; v < VOCAB_SIZE; v++) logits[v] += LONG_CONTEXT_WEIGHT * longIt->second[v];
			}
			if (lineLen < 22) logits[newlineIdx] -= 3.0f;
			if (lineLen > 52) logits[newlineIdx] += 2.5f;
			if (lineLen > 68) logits[newlineIdx] += 5.0f;
			if (history.size() >= 3) {
				size_t recentStart = history.size() > 180 ? history.size() - 180 : 0;
				for (size_t v = 0; v < VOCAB_SIZE; v++) {
					int repeats = 0;
					for (size_t k = recentStart; k + 3 < history.size(); k++) {
						if (history[k] == prev3 && history[k + 1] == prev2
							&& history[k + 2] == prev && history[k + 3] == static_cast<int>(v)) {
							repeats++;
						}
					}
