 ...
 *
 * Demonstrates neighborhood processing using nested For loops
 * inside a Kernel2D. Each thread computes the average of a 3x3
 * region centered at its pixel, with edge clamping.
 *
 * Techniques shown:
 * - Nested For loops inside kernels for local convolution
 * - Clamp for boundary-safe access
 * - Buffer<int> packed RGBA processing
...
int main() {
	...
	for (int y = 0; y < HEIGHT; ++y) {
		for (int x = 0; x < WIDTH; ++x) {
			...
			int expected = ((sum_a / KERNEL_AREA) << 24)
						 | ((sum_r / KERNEL_AREA) << 16)
						 | ((sum_g / KERNEL_AREA) << 8)
						 | (sum_b / KERNEL_AREA);
