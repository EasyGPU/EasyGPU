		...
		If(!HitWorld(cur, 0.001f, 1000.0f, rec, rng), [&]() { Break(); }).Else([&]() {
			If(rec.mat().type() == 2, [&]() {
				...
				If(rec.mat().type() == 0, [&]() {
					...
					Float3	 lightPos  = MakeFloat3((Random(rng) - 0.5f) * 0.5f, 0.745f, (Random(rng) - 0.5f) * 0.5f);
...
int main() {
	try {
		...
		Kernel::Kernel2D kernel([&](Int &px, Int &py) {
			...
			Float3	  total =
				MakeFloat3(prev4.x() + sampleColor.x(), prev4.y() + sampleColor.y(), prev4.z() + sampleColor.z());
