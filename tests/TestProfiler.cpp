...
Kernel::Kernel1D kernel1("my_kernel", [&](Var<int> &id) { Var<int> x = id; });
ASSERT(kernel1.GetName() == "my_kernel");
std::cout << "  ✓ Constructor naming works: " << kernel1.GetName() << "\n";

// Test default name
Kernel::Kernel1D kernel2([&](Var<int> &id) { Var<int> x = id; });
...
Kernel::Kernel2D kernel2d_named("my_2d_kernel", [&](Var<int> &x, Var<int> &y) { Var<int> sum = x + y; });
ASSERT(kernel2d_named.GetName() == "my_2d_kernel");
std::cout << "  ✓ 2D kernel naming works: " << kernel2d_named.GetName() << "\n";

Kernel::Kernel3D kernel3d_named("my_3d_kernel",
								[&](Var<int> &x, Var<int> &y, Var<int> &z) { Var<int> sum = x + y + z; });
