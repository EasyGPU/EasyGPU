...
for (int i = 0; i < W * H * 4; ++i) {
	if (std::abs(uploadPixels[i] - downloadPixels[i]) > 2) {
		match = false;
		std::cout << "Mismatch at byte " << i << ": uploaded " << (int)uploadPixels[i] << ", downloaded "
				  << (int)downloadPixels[i];
		break;
	}
}
...
for (int i = 0; i < W * H && correct; ++i) {
	...
	if (std::abs(r - 55) > 5 || std::abs(g - 155) > 5 || std::abs(b - 205) > 5 || a != 255) {
		correct = false;
...
for (int y = 0; y < H && correct; y += 8) {
	for (int x = 0; x < W && correct; x += 8) {
		...
		if (std::abs(r - expectedR) > 10 || std::abs(g - expectedG) > 10) {
			correct = false;
			std::cout << "Pixel (" << x << "," << y << ") mismatch: got (" << (int)r << "," << (int)g
...
GPU::Kernel::Kernel2D kernel(
	[&](Var<int> &idx, Var<int> &idy) {
		...
		auto	   getGray = [&](Var<int> x, Var<int> y) -> Expr<float> { return input.Read(x, y).x(); };
