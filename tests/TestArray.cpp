...
GPU::Kernel::Kernel1D kernel(
	[&](Var<int> &id) {
		...
		For(0, 10, [&](Int &i) { localArr[i] = Expr<float>(i) * 2.0f; });
...
for (int i = 0; i < 10; i++) {
	...
	if (std::abs(result[i] - expected) > 0.0001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": got " << result[i] << ", expected " << expected << "\n";
		break;
	}
}
...
for (int i = 0; i < N; i++) {
	if (std::abs(result[i] - expected[i]) > 0.0001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": got " << result[i] << ", expected " << expected[i] << "\n";
		break;
	}
}
...
GPU::Kernel::Kernel1D kernel(
	[&](Var<int> &id) {
		...
		// Initialize to zero
		For(0, NUM_BINS, [&](Int &j) { localHist[j] = MakeInt(0); });
...
for (int i = 0; i < NUM_BINS; i++) {
	if (result[i] != expected[i]) {
		correct = false;
		std::cout << "Mismatch at bin " << i << ": got " << result[i] << ", expected " << expected[i] << "\n";
		break;
	}
}
...
GPU::Kernel::Kernel1D kernel(
	[&](Var<int> &id) {
		...
		For(0, 4, [&](Int &i) { sum = sum + vectors[i]; });
...
for (int i = 0; i < 4; i++) {
	if (result[i].Length() > 0.0001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": got (" << result[i].x << ", " << result[i].y << ", " << result[i].z
				  << ")\n";
		break;
	}
}
...
GPU::Kernel::Kernel1D kernel(
	[&](Var<int> &id) {
		...
		For(0, 5, [&](Int &i) { arr1[i] = Expr<float>(i); });
