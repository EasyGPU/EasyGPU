...
Callable<float(float)> Square = [](Var<float> x) { Return(x * x); };
...
for (size_t i = 0; i < N; i++) {
	if (std::abs(outputData[i] - expectedData[i]) > 0.0001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": got " << outputData[i] << ", expected " << expectedData[i] << "\n";
		break;
	}
}
...
for (size_t i = 0; i < N; i++) {
	...
	if (std::abs(result1[i] - expected1) > 0.001f || std::abs(result2[i] - expected2) > 0.001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": r1=" << result1[i] << " expected1=" << expected1 << ", r2=" << result2[i]
				  << " expected2=" << expected2 << "\n";
		break;
	}
	...
	if (std::abs(result1[i] + result2[i]) > 0.001f) {
		correct = false;
		std::cout << "Sign mismatch at " << i << ": r1=" << result1[i] << " r2=" << result2[i] << "\n";
		break;
	}
}
...
for (size_t i = 0; i < N; i++) {
	...
	if (std::abs(outputData[i] - expected) > 0.001f) {
		correct = false;
		std::cout << "Mismatch at " << i << ": got " << outputData[i] << ", expected " << expected << "\n";
		break;
	}
}
...
for (size_t i = 0; i < N; i++) {
	...
	if (std::abs(len - 1.0f) > 0.01f) {
		correct = false;
		std::cout << "Length mismatch at " << i << ": got " << len << ", expected ~1.0\n";
		break;
	}
}
...
for (size_t i = 0; i < N; i++) {
	...
	if (!isPowerOfTwo || !isGE || !isMinimal) {
		correct = false;
		std::cout << "Mismatch at " << i << ": input=" << input << " output=" << output << "\n";
		break;
	}
}
...
Callable<Vec4(IR::Value::TextureRef<PixelFormat::RGBA8>, int, int)> SampleTexture =
	[](IR::Value::TextureRef<PixelFormat::RGBA8> img, Var<int> x, Var<int> y) { Return(img.Read(x, y)); };
...
Callable<Vec4(IR::Value::TextureSampler2D<PixelFormat::RGBA8>, IR::Value::TextureSampler2D<PixelFormat::RGBA8>, float,
			  float)>
	BlendTextures = [](IR::Value::TextureSampler2D<PixelFormat::RGBA8> img1,
					   IR::Value::TextureSampler2D<PixelFormat::RGBA8> img2, Var<float> u, Var<float> v) {
