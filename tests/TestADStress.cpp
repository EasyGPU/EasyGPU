...
#define CHECK_MIN_COUNT(str, sub, minCount)                                                                            \
	{                                                                                                                  \
		...
		if (count < (size_t)(minCount)) {                                                                              \
			throw std::runtime_error("Expected at least " + std::to_string(minCount) + " occurrences of '" + sub +     \
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	y = 0.0f;
	Var<bool> c1;
	c1 = x > Expr<float>(0.0f);
	Var<bool> c2;
	c2 = x > Expr<float>(1.0f);
	Var<bool> c3;
	c3 = x > Expr<float>(2.0f);
	Var<bool> c4;
	c4 = x > Expr<float>(3.0f);
	Var<bool> c5;
	c5 = x > Expr<float>(4.0f);
	If(c1, [&] {
		y = y + Expr<float>(1.0f);
		If(c2, [&] {
			y = y + Expr<float>(2.0f);
							...
							If(x > Expr<float>(6.0f), [&] {
								...
								If(x > Expr<float>(7.0f), [&] {
									...
									If(x > Expr<float>(8.0f), [&] {
										y = y + Expr<float>(9.0f);
										If(x > Expr<float>(9.0f), [&] { y = y + Expr<float>(10.0f); });
									});
								});
							});
						});
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	If(x > Expr<float>(0.0f), [&] {
		For(0, 5, 1, [&](Var<int> &i) {
			...
			If(sum > Expr<float>(5.0f), [&] { sum = sum + Expr<float>(1.0f); });
		});
		sum = sum * Expr<float>(x);
	});
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<float> y;
	y = 0.0f;
	If(x > Expr<float>(10.0f), [&] { y = y + Expr<float>(1.0f); })
		.Elif(x > Expr<float>(8.0f), [&] { y = y + Expr<float>(2.0f); })
		.Elif(x > Expr<float>(6.0f), [&] { y = y + Expr<float>(3.0f); })
		.Elif(x > Expr<float>(4.0f), [&] { y = y + Expr<float>(4.0f); })
		.Else([&] { y = y + Expr<float>(5.0f); });
	tape.MarkLoss(y.VarName(), "float");
});
ASSERT(!r.backwardCode.empty());
CHECK_CONTAINS(r.backwardCode, "if (");
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<float> y;
	y = 0.0f;
	If((a > b) && (b > c), [&] { y = a * b + c; }).Else([&] { y = a + b * c; });
	tape.MarkLoss(y.VarName(), "float");
});
ASSERT(!r.backwardCode.empty());
CHECK_CONTAINS(r.backwardCode, "if (");
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<float> x;
	x = 2.0f;
	For(0, 10, 1, [&](Var<int> &i) { accumulator = accumulator + x * Expr<float>(float(3)); });
...
Callable<float(float)> f0([](Var<float> x) {
	Var<float> r = x + Expr<float>(1.0f);
	Return(r);
});
Callable<float(float)> f1([](Var<float> x) {
	Var<float> r = x * Expr<float>(2.0f);
	Return(r);
});
Callable<float(float)> f2([](Var<float> x) {
	Var<float> r = x - Expr<float>(3.0f);
	Return(r);
});
Callable<float(float)> f3([](Var<float> x) {
	Var<float> r = x / Expr<float>(4.0f);
	Return(r);
});
Callable<float(float)> f4([](Var<float> x) {
	Var<float> r = x + Expr<float>(5.0f);
	Return(r);
});
Callable<float(float)> f5([](Var<float> x) {
	Var<float> r = x * Expr<float>(6.0f);
	Return(r);
});
Callable<float(float)> f6([](Var<float> x) {
	Var<float> r = x - Expr<float>(7.0f);
	Return(r);
});
Callable<float(float)> f7([](Var<float> x) {
	Var<float> r = x / Expr<float>(8.0f);
	Return(r);
});
Callable<float(float)> f8([](Var<float> x) {
	Var<float> r = x + Expr<float>(9.0f);
	Return(r);
});
Callable<float(float)> f9([](Var<float> x) {
	Var<float> r = x * Expr<float>(10.0f);
	Return(r);
});
...
Callable<float(float, float)> reluLike([](Var<float> a, Var<float> b) {
	Var<float> r;
	If(a > b, [&] { r = a * Expr<float>(b); }).Else([&] { r = a + Expr<float>(b); });
	Return(r);
});
...
Callable<float(float)> add1C([](Var<float> x) {
	Var<float> r = x + Expr<float>(1.0f);
	Return(r);
});
Callable<float(float)> mul2C([](Var<float> x) {
	Var<float> r = x * Expr<float>(2.0f);
	Return(r);
});
Callable<float(float)> sub3C([](Var<float> x) {
	Var<float> r = x - Expr<float>(3.0f);
	Return(r);
});
...
auto r = RunADParamTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	Var<float> p0;
	p0 = 1.0f;
	tape.RegisterParameter(p0.VarName(), "float");
	Var<float> p1;
	p1 = 2.0f;
	tape.RegisterParameter(p1.VarName(), "float");
	Var<float> p2;
	p2 = 3.0f;
	tape.RegisterParameter(p2.VarName(), "float");
	Var<float> p3;
	p3 = 4.0f;
	tape.RegisterParameter(p3.VarName(), "float");
...
auto r = RunADParamTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	Var<float> w0;
	w0 = 1.0f;
	tape.RegisterParameter(w0.VarName(), "float");
	Var<float> w1;
	w1 = 2.0f;
	tape.RegisterParameter(w1.VarName(), "float");
	Var<float> w2;
	w2 = 3.0f;
	tape.RegisterParameter(w2.VarName(), "float");
	Var<float> x0;
	x0 = 0.5f;
	Var<float> x1;
	x1 = 1.5f;
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<Vec3> b;
	b = Vec3(0.0f, 1.0f, 2.0f);
	Var<Vec3> c;
	c = Vec3(3.0f, 2.0f, 1.0f);
	Var<Vec3> d;
	d				= Vec3(1.0f, 1.0f, 1.0f);
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<float> y;
	y = 1.0f;
	If(x > Expr<float>(0.0f), [&] { y = y * Expr<float>(x); });
	tape.MarkLoss(y.VarName(), "float");
});
ASSERT(!r.backwardCode.empty());
CHECK_CONTAINS(r.backwardCode, "if (");
...
Callable<float(float)> mulR([](Var<float> x) {
	Var<float> r = x * Expr<float>(2.0f);
	Return(r);
});
