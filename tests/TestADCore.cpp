...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	Var<float> x; x = 0.5f;
	Var<float> lo; lo = 0.0f;
	Var<float> hi; hi = 1.0f;
...
auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
	...
	Var<float> w1; w1 = 0.5f;
	Var<float> w2; w2 = -0.3f;
...
END_TEST
	...
	TEST(ad_if_inside_for)
		auto r = RunADTest([](Var<int> &id, GPU::AD::GradientTape &tape) {
			Var<float> sum; sum = 0.0f;
			Var<float> x; x = 1.0f;
			Var<float> threshold; threshold = 0.5f;
...
TEST(ad_kernel_merged_with_gradbufs)
	...
	gb.paramName = "v1"; gb.glslType = "float"; gb.binding = 10; grads.push_back(gb);
	gb.paramName = "v2"; gb.glslType = "float"; gb.binding = 11; grads.push_back(gb);
	std::string combined = GPU::AD::MergeForwardBackward(r.forwardCode, body, grads);
...
TEST(ad_inspector_3d_basic)
	GPU::AD::AdjointInspector3D inspector([](Var<int> &idX, Var<int> &idY, Var<int> &idZ, GPU::AD::AdjointContext &ctx) {
		Var<float> a; a = 2.0f;
		Var<float> b; b = 3.0f;
		Var<float> c = a + b;
		ctx.MarkLoss<float>(c.VarName());
...
TEST(ad_kernel_2d_basic)
	GPU::AD::AdjointKernel2D kernel([](Var<int> &idX, Var<int> &idY, GPU::AD::AdjointContext &ctx) {
		Var<float> a; a = 2.0f;
		Var<float> b; b = 3.0f;
		Var<float> c = a + b;
		ctx.MarkLoss<float>(c.VarName());
