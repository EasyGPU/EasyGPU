...
public:
	 ...
	 */
	template <typename T>
	void RegisterParameter(const IR::Value::Var<T> &var) {
	 ...
	 */
	template <typename T>
	void MarkLoss(const IR::Value::Var<T> &var) {
	...
	template <typename T>
	void MarkLoss(const std::string &name) {
		_tape.MarkLoss(name, TypeName<T>());
	}

	/** Get the underlying gradient tape. */
	GradientTape &Tape() { return _tape; }
...
public:
	 ...
	 */
	AdjointInspector1D(Func &&kernelFunc, int workSizeX = 256)
	...
	std::string GetForwardCode() const { return _forwardKernel->GetCode(); }

	/** Get the backward-pass GLSL code. */
	std::string GetBackwardCode() const { return _backwardCode; }
	...
	std::string GetTapeSummary() const {
		...
		for (size_t i = 0; i < _tape.Size(); ++i) {
			...
			if (!e.intrinsicName.empty()) summary += " fn=" + e.intrinsicName;
			summary += " ins:";
			for (const auto &in : e.inputs) summary += in.name + ",";
			summary += "\n";
		}
		return summary;
	}
	...
	void PrintTape() { std::cout << GetTapeSummary(); }

	/** Get the underlying gradient tape. */
	const GradientTape &Tape() const { return _tape; }

	/** Get the adjoint table after generation. */
	const AdjointTable &Adjoints() const { return _generator.GetAdjointTable(); }

	/** Check if the backward code was generated successfully. */
	bool HasBackwardCode() const { return !_backwardCode.empty(); }

	/** Get the work group size. */
	int WorkSizeX() const { return _workSizeX; }
