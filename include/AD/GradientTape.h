...
public:
	...
	const std::optional<TapeVar> &LossVar() const { return _lossVar; }
	...
	void BeginForLoop(const std::string &varName, const std::string &start, const std::string &end, const std::string &step);
	...
	const GradientTape &SubTape(int index) const { return *_subTapes[index]; }

	/** Get all sub-tapes. */
	const auto &SubTapes() const { return _subTapes; }

	// ---- Access ----------------------------------------------------------

	size_t			  Size() const { return _entries.size(); }
	const TapeEntry  &operator[](int32_t i) const { return _entries[i]; }
	const auto		  &Entries() const { return _entries; }
