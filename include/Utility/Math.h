 ...
 */
template <typename T> std::unique_ptr<IR::Node::Node> WrapNode(const IR::Value::Expr<T> &expr) {
...
template <typename T> struct ValueTypeOf<IR::Value::Expr<T>> {
	using type = T;
};
template <typename T> struct ValueTypeOf<IR::Value::Var<T>> {
	using type = T;
};
template <typename T> using ValueTypeOf_t = typename ValueTypeOf<std::remove_cvref_t<T>>::type;

// Trait to detect if T is Var<...> (has conversion to Expr but is not Expr itself)
template <typename T> struct IsVar : std::false_type {};
template <typename T> struct IsVar<IR::Value::Var<T>> : std::true_type {};

// Trait to detect if T is a CPU-side Vec type (Math::Vec2/3/4, IVec2/3/4)
template <typename T> struct IsCpuVec : std::false_type {};
...
template <typename T> [[nodiscard]] inline auto ToExpr(T &&val) -> IR::Value::ExprBase {
...
template <typename T1> std::vector<std::unique_ptr<IR::Node::Node>> BuildParams(T1 &&arg1) {
...
// Two parameters
template <typename T1, typename T2> std::vector<std::unique_ptr<IR::Node::Node>> BuildParams(T1 &&arg1, T2 &&arg2) {
	std::vector<std::unique_ptr<IR::Node::Node>> params;
	AddParam(params, std::forward<T1>(arg1));
...
template <typename T> using expr_t = IR::Value::Expr<ValueTypeOf_t<T>>;
} // namespace Detail
...
inline IR::Value::Expr<Vec2> Clamp(const IR::Value::Expr<Vec2> &x, const IR::Value::Expr<Vec2> &minVal,
								   const IR::Value::Expr<Vec2> &maxVal) {
	return MakeCall<Vec2>("clamp", Detail::BuildParams(x, minVal, maxVal));
}
inline IR::Value::Expr<Vec3> Clamp(const IR::Value::Expr<Vec3> &x, const IR::Value::Expr<Vec3> &minVal,
								   const IR::Value::Expr<Vec3> &maxVal) {
	return MakeCall<Vec3>("clamp", Detail::BuildParams(x, minVal, maxVal));
}
inline IR::Value::Expr<Vec4> Clamp(const IR::Value::Expr<Vec4> &x, const IR::Value::Expr<Vec4> &minVal,
								   const IR::Value::Expr<Vec4> &maxVal) {
...
inline IR::Value::Expr<int> Clamp(const IR::Value::Expr<int> &x, const IR::Value::Expr<int> &minVal,
								  const IR::Value::Expr<int> &maxVal) {
	return MakeCall<int>("clamp", Detail::BuildParams(x, minVal, maxVal));
}
inline IR::Value::Expr<IVec2> Clamp(const IR::Value::Expr<IVec2> &x, const IR::Value::Expr<IVec2> &minVal,
									const IR::Value::Expr<IVec2> &maxVal) {
	return MakeCall<IVec2>("clamp", Detail::BuildParams(x, minVal, maxVal));
}
inline IR::Value::Expr<IVec3> Clamp(const IR::Value::Expr<IVec3> &x, const IR::Value::Expr<IVec3> &minVal,
									const IR::Value::Expr<IVec3> &maxVal) {
	return MakeCall<IVec3>("clamp", Detail::BuildParams(x, minVal, maxVal));
}
inline IR::Value::Expr<IVec4> Clamp(const IR::Value::Expr<IVec4> &x, const IR::Value::Expr<IVec4> &minVal,
									const IR::Value::Expr<IVec4> &maxVal) {
...
template <typename X, typename Min, typename Max> [[nodiscard]] inline auto Clamp(X &&x, Min &&minVal, Max &&maxVal) {
	using T = Detail::ValueTypeOf_t<X>;
	return MakeCall<T>("clamp", Detail::BuildParams(Detail::ToExpr(std::forward<X>(x)),
...
inline IR::Value::Expr<Vec2> Mix(const IR::Value::Expr<Vec2> &x, const IR::Value::Expr<Vec2> &y,
								 const IR::Value::Expr<Vec2> &a) {
	return MakeCall<Vec2>("mix", Detail::BuildParams(x, y, a));
}
inline IR::Value::Expr<Vec3> Mix(const IR::Value::Expr<Vec3> &x, const IR::Value::Expr<Vec3> &y,
								 const IR::Value::Expr<Vec3> &a) {
	return MakeCall<Vec3>("mix", Detail::BuildParams(x, y, a));
}
inline IR::Value::Expr<Vec4> Mix(const IR::Value::Expr<Vec4> &x, const IR::Value::Expr<Vec4> &y,
								 const IR::Value::Expr<Vec4> &a) {
...
inline IR::Value::Expr<Vec2> Smoothstep(const IR::Value::Expr<Vec2> &edge0, const IR::Value::Expr<Vec2> &edge1,
										const IR::Value::Expr<Vec2> &x) {
	return MakeCall<Vec2>("smoothstep", Detail::BuildParams(edge0, edge1, x));
}
inline IR::Value::Expr<Vec3> Smoothstep(const IR::Value::Expr<Vec3> &edge0, const IR::Value::Expr<Vec3> &edge1,
										const IR::Value::Expr<Vec3> &x) {
	return MakeCall<Vec3>("smoothstep", Detail::BuildParams(edge0, edge1, x));
}
inline IR::Value::Expr<Vec4> Smoothstep(const IR::Value::Expr<Vec4> &edge0, const IR::Value::Expr<Vec4> &edge1,
										const IR::Value::Expr<Vec4> &x) {
...
template <typename E0, typename E1, typename X> [[nodiscard]] inline auto Smoothstep(E0 &&edge0, E1 &&edge1, X &&x) {
	using T = Detail::ValueTypeOf_t<X>;
	return MakeCall<T>("smoothstep", Detail::BuildParams(Detail::ToExpr(std::forward<E0>(edge0)),
...
inline IR::Value::Expr<Vec2> Faceforward(const IR::Value::Expr<Vec2> &N, const IR::Value::Expr<Vec2> &I,
										 const IR::Value::Expr<Vec2> &Nref) {
	return MakeCall<Vec2>("faceforward", Detail::BuildParams(N, I, Nref));
}
inline IR::Value::Expr<Vec3> Faceforward(const IR::Value::Expr<Vec3> &N, const IR::Value::Expr<Vec3> &I,
										 const IR::Value::Expr<Vec3> &Nref) {
	return MakeCall<Vec3>("faceforward", Detail::BuildParams(N, I, Nref));
}
inline IR::Value::Expr<Vec4> Faceforward(const IR::Value::Expr<Vec4> &N, const IR::Value::Expr<Vec4> &I,
										 const IR::Value::Expr<Vec4> &Nref) {
...
inline IR::Value::Expr<Vec2> Refract(const IR::Value::Expr<Vec2> &I, const IR::Value::Expr<Vec2> &N,
									 const IR::Value::Expr<float> &eta) {
	return MakeCall<Vec2>("refract", Detail::BuildParams(I, N, eta));
}
inline IR::Value::Expr<Vec3> Refract(const IR::Value::Expr<Vec3> &I, const IR::Value::Expr<Vec3> &N,
									 const IR::Value::Expr<float> &eta) {
	return MakeCall<Vec3>("refract", Detail::BuildParams(I, N, eta));
}
inline IR::Value::Expr<Vec4> Refract(const IR::Value::Expr<Vec4> &I, const IR::Value::Expr<Vec4> &N,
									 const IR::Value::Expr<float> &eta) {
