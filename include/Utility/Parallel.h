...
[[nodiscard]] IR::Value::Expr<T> WorkgroupReduce(
	IR::Value::SharedMemory<T, N>& shared,
	...
	if constexpr (N >= 512) {
		If(lid < 256, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 256]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 256) {
		If(lid < 128, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 128]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 128) {
		If(lid < 64, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 64]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 64) {
		If(lid < 32, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 32]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 32) {
		If(lid < 16, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 16]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 16) {
		If(lid < 8, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 8]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 8) {
		If(lid < 4, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 4]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 4) {
		If(lid < 2, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 2]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
	if constexpr (N >= 2) {
		If(lid < 1, [&]() {
			shared[lid] = op(Expr<T>(shared[lid]), Expr<T>(shared[lid + 1]));
		});
		Kernel::KernelBase::WorkgroupBarrier();
	}
 ...
 */
template <typename T, int N, typename Op>
[[nodiscard]] IR::Value::Var<T> WorkgroupScanInclusive(
	IR::Value::SharedMemory<T, N>& shared,
	const IR::Value::Expr<T>& value,
	Op op
) {
 ...
 */
template <typename T, int N, typename Op>
[[nodiscard]] IR::Value::Var<T> WorkgroupScanExclusive(
	IR::Value::SharedMemory<T, N>& shared,
	const IR::Value::Expr<T>& value,
	T identity,
	Op op
