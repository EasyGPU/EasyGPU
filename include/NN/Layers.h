...
public:
	...
	void Forward(const IR::Value::BufferRef<T> &input,
		...
		GPU::Flow::For(MakeInt(0), MakeInt(kOut), [&](IR::Value::Var<int> &o) {
			...
			GPU::Flow::For(MakeInt(0), MakeInt(kIn), [&](IR::Value::Var<int> &i) {
				sum += weightRef_(o, i) * input[threadId * kIn + i];
			});
	...
	Tensor<T, InFeatures, OutFeatures> &Weight() { return weight_; }
	Tensor<T, OutFeatures> &Bias() { return bias_; }
	const Tensor<T, InFeatures, OutFeatures> &Weight() const { return weight_; }
	const Tensor<T, OutFeatures> &Bias() const { return bias_; }

	T *WeightData() { return weight_.Data(); }
	T *BiasData() { return bias_.Data(); }

	static constexpr size_t InputDim() { return InFeatures; }
	static constexpr size_t OutputDim() { return OutFeatures; }
	size_t ParamCount() const { return InFeatures * OutFeatures + OutFeatures; }
...
public:
	...
	void Setup() {} // no-op for stateless activations
...
public:
	...
	void Forward(const IR::Value::BufferRef<T> &in,
		...
		GPU::Flow::For(MakeInt(0), MakeInt(n), [&](IR::Value::Var<int> &i) {
			...
			out[threadId * n + i] = MakeFloat(2.0f) / (MakeFloat(1.0f) + GPU::Math::Exp(MakeFloat(0.0f) - twoX)) - MakeFloat(1.0f);
		});
	}
...
public:
	...
	template <size_t I>
	auto &Get() { return std::get<I>(layers_); }

	template <size_t I>
	const auto &Get() const { return std::get<I>(layers_); }
...
private:
	template <size_t I>
	void ForwardStep(const IR::Value::BufferRef<T> &in,
					 const IR::Value::Var<int> &threadId,
