...
template <typename T> IR::Value::Var<T> ApplyFusedActivation(const IR::Value::Var<T> &x, FusedActivation activation) {
	switch (activation) {
	case FusedActivation::ReLU:
		return GPU::Math::Max(x, MakeFloat(0.0f));
	case FusedActivation::Tanh:
		return MakeFloat(2.0f) / (MakeFloat(1.0f) + GPU::Math::Exp(MakeFloat(0.0f) - x * MakeFloat(2.0f))) -
			   MakeFloat(1.0f);
...
public:
	...
	static constexpr size_t ParamCount() {
		return HiddenFeatures * InFeatures + HiddenFeatures + OutFeatures * HiddenFeatures + OutFeatures;
	}
