...
public:
	explicit FusedMLP2Trainer(unsigned seed = 42)
		: w1_(W1Size, Runtime::BufferMode::ReadWrite),
		  b1_(B1Size, Runtime::BufferMode::ReadWrite),
		  w2_(W2Size, Runtime::BufferMode::ReadWrite),
		  b2_(B2Size, Runtime::BufferMode::ReadWrite),
		  gw1_(W1Size, Runtime::BufferMode::ReadWrite),
		  gb1_(B1Size, Runtime::BufferMode::ReadWrite),
		  gw2_(W2Size, Runtime::BufferMode::ReadWrite),
		  gb2_(B2Size, Runtime::BufferMode::ReadWrite),
		  m_(TotalParams, Runtime::BufferMode::ReadWrite),
		  v_(TotalParams, Runtime::BufferMode::ReadWrite),
	...
	~FusedMLP2Trainer() { Release(); }

	FusedMLP2Trainer(const FusedMLP2Trainer &) = delete;
	FusedMLP2Trainer &operator=(const FusedMLP2Trainer &) = delete;

	Runtime::Buffer<float> &W1() { return w1_; }
	Runtime::Buffer<float> &B1() { return b1_; }
	Runtime::Buffer<float> &W2() { return w2_; }
	Runtime::Buffer<float> &B2() { return b2_; }
	Runtime::Buffer<float> &LossBuffer() { return loss_; }

	static constexpr size_t ParameterCount() { return TotalParams; }
	...
	void SetWeights(const std::vector<float> &w1,
		...
		if (w1.size() != W1Size || b1.size() != B1Size ||
			w2.size() != W2Size || b2.size() != B2Size)
	...
	std::vector<float> DownloadW1() { return DownloadVector(w1_, W1Size); }
	std::vector<float> DownloadB1() { return DownloadVector(b1_, B1Size); }
	std::vector<float> DownloadW2() { return DownloadVector(w2_, W2Size); }
	std::vector<float> DownloadB2() { return DownloadVector(b2_, B2Size); }
	float DownloadLoss() {
	...
	void Forward(Runtime::Buffer<float> &input,
		...
		if (!backend) throw std::runtime_error("FusedMLP2Trainer backend not available");
	...
	void TrainMSE(Runtime::Buffer<float> &input,
				  ...
				  float eps = 1e-8f,
				  bool sync = false) {
		if (batch == 0) throw std::invalid_argument("FusedMLP2Trainer::TrainMSE batch must be > 0");
	...
	static std::string ForwardShaderSource(size_t batch) { return BuildForwardShader(batch); }
	static std::string TrainingShaderSource(size_t batch) { return BuildTrainShader(batch); }
	static std::string UpdateShaderSource() { return BuildUpdateShader(); }
...
private:
	...
	static void Bind(Backend::ResourceBinding &rb, uint32_t binding,
					 Backend::BufferHandle handle, bool readOnly) {
		rb.binding = binding;
...
#define ATOMIC_ADD_FLOAT(target, value) do { \
	...
	static void AppendForwardMath(std::string &src,
...
void main() {
	...
	Backend::PipelineHandle CreatePipeline(Backend::ShaderHandle shader, uint32_t bindings, uint32_t readOnlyMask) {
		...
		for (uint32_t i = 0; i < bindings; i++)
			pipelineDesc.resources.push_back({i, Backend::BindingType::Buffer, Backend::PixelFormat::RGBA8,
											  (readOnlyMask & (1u << i)) != 0});
	...
	void EnsureForwardPipeline(size_t batch) {
		if (forwardPipeline_ != Backend::INVALID_PIPELINE_HANDLE && forwardBatch_ == batch) return;
		Runtime::Context::GetInstance().MakeCurrent();
		auto *backend = Runtime::Context::GetBackend();
		if (forwardPipeline_ != Backend::INVALID_PIPELINE_HANDLE) backend->DestroyPipeline(forwardPipeline_);
		if (forwardShader_ != Backend::INVALID_SHADER_HANDLE) backend->DestroyShader(forwardShader_);
	...
	void EnsureTrainPipeline(size_t batch) {
		if (trainPipeline_ != Backend::INVALID_PIPELINE_HANDLE && trainBatch_ == batch) return;
		Runtime::Context::GetInstance().MakeCurrent();
		auto *backend = Runtime::Context::GetBackend();
		if (trainPipeline_ != Backend::INVALID_PIPELINE_HANDLE) backend->DestroyPipeline(trainPipeline_);
		if (trainShader_ != Backend::INVALID_SHADER_HANDLE) backend->DestroyShader(trainShader_);
	...
	void EnsureUpdatePipeline() {
		if (updatePipeline_ != Backend::INVALID_PIPELINE_HANDLE) return;
		Runtime::Context::GetInstance().MakeCurrent();
	...
	void DispatchClear(bool sync) {
		...
		backend->Dispatch(static_cast<uint32_t>((std::max({W1Size, B1Size, W2Size, B2Size}) + WorkgroupSize - 1) / WorkgroupSize), 1, 1);
		backend->MemoryBarrier(Backend::BarrierType::Buffer);
		if (sync) backend->Finish();
	}
